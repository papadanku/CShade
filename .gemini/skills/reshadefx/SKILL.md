...
* `REFERENCE.md`: This document provides the core reference for the ReShadeFX shading language, which is based on DX9-style HLSL with extensions. It is the definitive guide to unique ReShadeFX features, including:
  ...
  * Uniform Variables: Outlines the comprehensive set of UI annotations (e.g., `ui_type`, `ui_min`, `ui_max`, `ui_step`, `ui_items`, `ui_label`, `ui_tooltip`, `ui_category`, `ui_category_closed`, `ui_category_toggle`, `ui_text`, `ui_spacing`, `ui_units`, `hidden`, `noedit`, `nosave`, `noreset`) for making variables user-configurable. It also lists annotations to request special runtime values via the `source` property (e.g., `frametime`, `timer`, `mousepoint`, `key`, `mousebutton`, `mousewheel`, `bufready_depth`, `overlay_open`, `overlay_active`, `overlay_hovered`, `screenshot`).
...
3. When explaining code or concepts, use bold text for keywords and refer to the provided files with citations for authority (e.g., "From `REFERENCE.md`").
