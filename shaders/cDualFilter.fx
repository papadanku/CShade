...
{
    float2 LocalTexCoord;;
    Basic_VS(ID, Position, LocalTexCoord);
    switch(_DownsampleMethod)
    {
        case 0: // 4x4 Box
            TexCoords[0] = LocalTexCoord.xyxy + float4(-1.0, -1.0, 1.0, 1.0) * TexelSize.xyxy;
            break;
        case 1: // 6x6 Tent
            TexCoords[0] = LocalTexCoord.xyyy + float4(-2.0, 2.0, 0.0, -2.0) * TexelSize.xyyy;
            TexCoords[1] = LocalTexCoord.xyyy + float4(0.0, 2.0, 0.0, -2.0) * TexelSize.xyyy;
            TexCoords[2] = LocalTexCoord.xyyy + float4(2.0, 2.0, 0.0, -2.0) * TexelSize.xyyy;
            break;
        case 2: // Jorge
            TexCoords[0] = LocalTexCoord.xyxy + float4(-1.0, -1.0, 1.0, 1.0) * TexelSize.xyxy;
            TexCoords[1] = LocalTexCoord.xyyy + float4(-2.0, 2.0, 0.0, -2.0) * TexelSize.xyyy;
            TexCoords[2] = LocalTexCoord.xyyy + float4(0.0, 2.0, 0.0, -2.0) * TexelSize.xyyy;
            TexCoords[3] = LocalTexCoord.xyyy + float4(2.0, 2.0, 0.0, -2.0) * TexelSize.xyyy;
            break;
        case 3: // Kawase
            TexCoords[0] = LocalTexCoord.xyxy;
            TexCoords[1] = LocalTexCoord.xyxy + float4(-1.0, -1.0, 1.0, 1.0) * TexelSize.xyxy;
            break;
    }
}
...
void Upsample_VS(in uint ID, inout float4 Position, inout float4 TexCoords[3], float2 TexelSize)
{
    ...
    {
        case 0: // 4x4 Box
            TexCoords[0] = LocalTexCoord.xyxy + float4(-0.5, -0.5, 0.5, 0.5) * TexelSize.xyxy;
            break;
        case 1: // 6x6 Tent
            TexCoords[0] = LocalTexCoord.xyyy + float4(-1.0, 1.0, 0.0, -1.0) * TexelSize.xyyy;
            TexCoords[1] = LocalTexCoord.xyyy + float4(0.0, 1.0, 0.0, -1.0) * TexelSize.xyyy;
            TexCoords[2] = LocalTexCoord.xyyy + float4(1.0, 1.0, 0.0, -1.0) * TexelSize.xyyy;
            break;
        case 2: // Kawase
            TexCoords[0] = LocalTexCoord.xyxy + float4(-0.5, -0.5, 0.5, 0.5) * TexelSize.xyxy;
            TexCoords[1] = LocalTexCoord.xxxy + float4(1.0, 0.0, -1.0, 0.0) * TexelSize.xxxy;
            TexCoords[2] = LocalTexCoord.xyyy + float4(0.0, 1.0, 0.0, -1.0) * TexelSize.xyyy;
            break;
    }
}
