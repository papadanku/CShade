    ...
    */
    ...
    void UnpackTex(in Texel Tx, in float4 Tex, in float2 Vectors, out UnpackedTex Output[4])
    {
        ...
        WarpPackedTex.xy = Tex.xy + (Vectors.xx * abs(Tx.Size.xx));
        // Warp vertical texture coordinates with vertical motion vector
        WarpPackedTex.zw = Tex.zw + (Vectors.yy * abs(Tx.Size.yy));
        ...
        Output[0].Tex = (Tex.xwww * TexMask) + Tx.LOD.xxxy;
        Output[1].Tex = (Tex.ywww * TexMask) + Tx.LOD.xxxy;
        Output[2].Tex = (Tex.xzzz * TexMask) + Tx.LOD.xxxy;
        Output[3].Tex = (Tex.yzzz * TexMask) + Tx.LOD.xxxy;

        Output[0].WarpedTex = (WarpPackedTex.xwww * TexMask) + Tx.LOD.xxxy;
        Output[1].WarpedTex = (WarpPackedTex.ywww * TexMask) + Tx.LOD.xxxy;
        Output[2].WarpedTex = (WarpPackedTex.xzzz * TexMask) + Tx.LOD.xxxy;
        Output[3].WarpedTex = (WarpPackedTex.yzzz * TexMask) + Tx.LOD.xxxy;
    }
    ...
    )
    {
        ...
        UnpackTex(Tx, MainTex.xxyy + (float4(-0.5, 0.5, -0.5, 0.5) * abs(Tx.Size.xxyy)), Vectors, Pixel);

        // Sum windowed matrices

        [unroll]
        for(int i = 0; i < WindowSize; i++)
        {
            float4 G = tex2Dlod(SampleG, Pixel[i].Tex);
        ...
        else if((E[0] > EThreshold) && (E[1] > EThreshold))
        {
            Refine = true;
        }
        else
